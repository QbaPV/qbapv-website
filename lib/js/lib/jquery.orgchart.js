  ...
  OrgChart.prototype = {
    ...
    init: function (opts) {
      ...
      if ($.type(data) === 'object') {
        ...
        $chart.append('<i class="icon-circle-o-notch icon-spin spinner"></i>');
        $.ajax({
          'url': data,
          'dataType': 'json'
        })
    ...
    startLoading: function ($edge) {
      ...
      $edge.parent().append('<i class="icon-circle-o-notch icon-spin spinner"></i>')
    ...
    isInAction: function ($node) {
      return $node.children('.edge').attr('class').indexOf('icon-') > -1 ? true : false;
    },
    //
    ...
    toggleVNodes: function (event) {
      ...
      $toggleBtn.toggleClass('icon-plus-square icon-minus-square');
      if ($descendants.eq(0).is('.slide-up')) {
        ...
        $descendants.find('.toggleBtn').removeClass('icon-minus-square').addClass('icon-plus-square');
      }
    },
    //
    ...
    createNode: function (data) {
      ...
      if (opts.nodeTemplate) {
        ...
        if(data.relationship !== '001'){//Si se analiza el padre del arbol
      ...
      if (opts.verticalLevel && level >= opts.verticalLevel) {
        if ((level + 1) > opts.verticalLevel && Number(flags.substr(2,1))) {
          ...
          $nodeDiv.append('<i class="toggleBtn icon-' + icon + '-square"></i>');
        }
      } else {
        ...
        if(Number(flags.substr(2,1))) {
          $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa"></i>')
            .children('.title').prepend('<i class="'+ opts.parentNodeSymbol + ' symbol"></i>');
        }
      }
    ...
    buildHierarchy: function ($appendTo, data) {
      ...
      if (hasChildren) {
        ...
        if (isVerticalLayer) {
          ...
          var $upperLines = $('<tr class="lines' + isHidden + '"><td colspan="' + childrenData.length * 2 + '"><div class=""></div></td></tr>');
//          var $upperLines = $('<tr class="lines' + isHidden + '"><td colspan="' + childrenData.length * 2 + '"><div class="downLine"></div></td></tr>');
    ...
    addChildren: function ($node, data) {
      ...
      if (!$node.find('.symbol').length) {
        $node.children('.title').prepend('<i class="'+ this.options.parentNodeSymbol + ' symbol"></i>');
      }
      if (this.isInAction($node)) {
        this.switchVerticalArrow($node.children('.bottomEdge'));
      }
    ...
    export: function (exportFilename, exportFileextension) {
      ...
      if (!$mask.length) {
        $chartContainer.append('<div class="mask"><i class="icon-circle-o-notch icon-spin spinner"></i></div>');
      } else {
